...
`summary()`
Gives the mean, median, min, and max. As well as levels and sample size for each level of categorical variable.
```{r}
...
`select()`, `slice()`, `filter()`, `arrange()`, `mutate()`
data frame always comes first

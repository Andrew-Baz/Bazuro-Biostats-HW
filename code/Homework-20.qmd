...
```{r}
SOM <- soil |> group_by(Forest) |> mutate(mean_SOM = mean(SOM_percent)) |> mutate(sd_SOM = sd(SOM_percent)) |> slice(1)
##unsure if the slice was necessary here but without the slice it created a plot with dozens of boxes instead of error bars. I have included it below Chunk 5 under the name SOM_test
```
...
```{r}
SOM_test <- soil |> group_by(Forest) |> mutate(mean_SOM = mean(SOM_percent)) |> mutate(sd_SOM = sd(SOM_percent))

ggplot(SOM_test, aes(Forest, mean_SOM))+
  geom_errorbar(aes(ymax = mean_SOM + sd_SOM, ymin = mean_SOM - sd_SOM), width = 0.2)+
  geom_col(fill = "white", color = "black")+
  ylab("Mean percent soil organic matter")+
  xlab("Forest name")
...
I would state that SOM has the greatest median in the Kip Tract, followed by Donnerville, South Hammond, and finally Degrasse.
